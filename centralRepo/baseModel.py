...
class baseModel():
    ...
    def predict(self, data, sampler = None, lossFn = None,tsneEnable=False):
        '''
        ...
        with torch.no_grad():
            ...
            for d in dataLoader:
                ...
                if tsneEnable:
                    ...
                    embeddings = TSNE(n_components=2, random_state=19981127).fit_transform(feats.data.detach().cpu())
                    ax.scatter(embeddings[y == 0, 0], embeddings[y == 0, 1], c='royalblue', s=5)
    ...
    def calculateResults(self, yPredicted, yActual, classes = None):
        ...
        recall = recall_score(yActual, yPredicted, average='macro')
        precision = precision_score(yActual, yPredicted, average='macro')
        if classes is not None:
            cm = confusion_matrix(yActual, yPredicted, labels= classes)
            kappa = cohen_kappa_score(yActual, yPredicted, labels=classes)
            f1 = f1_score(yActual, yPredicted, labels=classes, average='macro')
        else:
        ...
        return {'acc': acc, 'recall':recall,'pre':precision,'cm': cm,'kappa':kappa,'f1':f1,}
