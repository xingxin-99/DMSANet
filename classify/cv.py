...
def cv(datasetId = None, network = None, nGPU = None, subTorun=None):
    ...
    config['outPath'] = os.path.join(config['outPath'],config['network'], randomFolder,'')
    # create the path
    ...
    for i, sub in enumerate(subs):
        ...
        # kappa
        trainKappaCV = [[r['kappa'] for r in result] for result in trainResultsCV]
        trainKappaCV = list(map(list, zip(*trainKappaCV)))
        trainKappaCV = [mean(data) for data in trainKappaCV]
        valKappaCV = [[r['kappa'] for r in result] for result in valResultsCV]
        valKappaCV = list(map(list, zip(*valKappaCV)))
        valKappaCV = [mean(data) for data in valKappaCV]
        testKappaCV = [[r['kappa'] for r in result] for result in testResultsCV]
        testKappaCV = list(map(list, zip(*testKappaCV)))
        testKappaCV = [mean(data) for data in testKappaCV]

        # f1
        trainF1CV = [[r['f1'] for r in result] for result in trainResultsCV]
        trainF1CV = list(map(list, zip(*trainF1CV)))
        trainF1CV = [mean(data) for data in trainF1CV]
        valF1CV = [[r['f1'] for r in result] for result in valResultsCV]
        valF1CV = list(map(list, zip(*valF1CV)))
        valF1CV = [mean(data) for data in valF1CV]
        testF1CV = [[r['f1'] for r in result] for result in testResultsCV]
        testF1CV = list(map(list, zip(*testF1CV)))
        testF1CV = [mean(data) for data in testF1CV]

        # recall
        trainRecallCV = [[r['recall'] for r in result] for result in trainResultsCV]
        trainRecallCV = list(map(list, zip(*trainRecallCV)))
        trainRecallCV = [mean(data) for data in trainRecallCV]
        valRecallCV = [[r['recall'] for r in result] for result in valResultsCV]
        valRecallCV = list(map(list, zip(*valRecallCV)))
        valRecallCV = [mean(data) for data in valRecallCV]
        testRecallCV = [[r['recall'] for r in result] for result in testResultsCV]
        testRecallCV = list(map(list, zip(*testRecallCV)))
        testRecallCV = [mean(data) for data in testRecallCV]

        # precision
        trainPreCV = [[r['pre'] for r in result] for result in trainResultsCV]
        trainPreCV = list(map(list, zip(*trainPreCV)))
        trainPreCV = [mean(data) for data in trainPreCV]
        valPreCV = [[r['pre'] for r in result] for result in valResultsCV]
        valPreCV = list(map(list, zip(*valPreCV)))
        valPreCV = [mean(data) for data in valPreCV]
        testPreCV = [[r['pre'] for r in result] for result in testResultsCV]
        testPreCV = list(map(list, zip(*testPreCV)))
        testPreCV = [mean(data) for data in testPreCV]
        ...
        for iTemp, trainAc in enumerate(trainAccCV):
            temp1.append(
                {'acc': trainAc, 'pre': trainPreCV[iTemp], 'recall': trainRecallCV[iTemp], 'f1': trainF1CV[iTemp],
                 'kappa': trainKappaCV[iTemp], 'cm': trainCmCV[iTemp]})
            temp2.append(
                {'acc': valAccCV[iTemp], 'pre': valPreCV[iTemp], 'recall': valRecallCV[iTemp], 'f1': valF1CV[iTemp],
                 'kappa': valKappaCV[iTemp], 'cm': valCmCV[iTemp]})
            temp3.append(
                {'acc': testAccCV[iTemp], 'pre': testPreCV[iTemp], 'recall': testRecallCV[iTemp], 'f1': testF1CV[iTemp],
                 'kappa': testKappaCV[iTemp], 'cm': testCmCV[iTemp]})
    ...
    for i, res in enumerate(trainAcc):
        ...
        sheet1 = excelAddData(sheet1, [0, 0], ['SubId', 'trainAcc', 'valAcc', 'testAcc','trainKappa','valKappa','testKappa','trainF1','valF1','testF1','trainRecall','valRecall','testRecall','trainPre','valPre','testPre'])
